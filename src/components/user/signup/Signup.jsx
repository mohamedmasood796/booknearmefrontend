  ...
  return (
    ...
    <div className="container mx-auto">
        <div className="flex flex-col lg:flex-row w-10/12 lg:w-9/12 bg-white rounded-xl mx-auto shadow-lg overflow-hidden">
            <div className='w-full lg:w-1/2 flex flex-col items-center justify-center p-12 bg-no-repeat bg-cover bg-center' style={{ backgroundImage: `url(https://digital.ihg.com/is/image/ihg/holiday-inn-hotel-and-suites-clearwater-beach-6477439006-4x3)` }}>
            ...
            <div className='w-full lg:w-1/2 py-16 px-12'>
                ...
                <form>
                    <div className='mt-5'>
                        <input onChange={usernameOnChange} type="text" name="firstName" placeholder='Fullname' className='border border-gray-400 py-1 px-2 w-full ' />
                    ...
                    <div className='grid grid-cols-2 gap-5'>
                        <div>
                            {username && !isUserName ? <small className='text-red-500'>*Please enter first name</small> : null}
                            {!username && submit ? <small className='text-red-500'>Please enter a valid name</small> : null}
                        </div>
                        {/* <div>
                    ...
                    <div className='mt-5'>
                        <input onChange={emailOnChange} type="email" name="email" placeholder='Email' className='border border-gray-400 py-1 px-2 w-full' />
                    </div>
                    <div>
                        {email && !isEmail ? <small className='text-red-500'>*Please enter an email</small> : null}
                        {!email && submit ? <small className='text-red-500'>Please enter a valid email</small> : null}
                    </div>
                    <div className='mt-5'>
                        <input onChange={passwordOnChange} type="password" name="password" placeholder='Password' className='border border-gray-400 py-1 px-2 w-full' />
                    </div>
                    <div>
                        {password && !isPassword ? <small className='text-red-500'>*Please enter password</small> : null}
                        {!password && submit ? <small className='text-red-500'>Make password stronger</small> : null}
                    </div>
                    {/* <div className='mt-5'>
                    ...
                    <div className="mt-5">
                        <button onClick={handleSignup}  className='w-full bg-gray-800 py-3 text-center text-white'>Register Now</button>
                    </div>
                </form>
            </div>
        </div>
